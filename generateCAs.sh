...
# Create a temporary OpenSSL config for SubCA extensions
cat > "$OUTPUT_DIR/subca_ext.cnf" <<EOL
...
openssl x509 -req -in "$OUTPUT_DIR/$SUB_CA_NAME.csr" \
    -CA "$OUTPUT_DIR/$ROOT_CA_NAME.crt" -CAkey "$OUTPUT_DIR/$ROOT_CA_NAME.key" -CAcreateserial \
    -out "$OUTPUT_DIR/$SUB_CA_NAME.crt" -days $SUB_VALIDITY -sha256 \
    -extfile "$OUTPUT_DIR/subca_ext.cnf" \
    -passin pass:"$rootPass"

# Remove temporary config
rm "$OUTPUT_DIR/subca_ext.cnf"
...
# generate pem + crt
openssl pkcs12 -in certs/subCA.p12 -nokeys -nodes -out certs/test/subCA.pem -passin pass:"$subPass"
openssl pkcs12 -in certs/subCA.p12  -nodes -out certs/test/subCA.key -passin pass:"$subPass"

openssl pkcs12 -in certs/rootCA.p12  -nodes -out certs/test/rootCA.key -passin pass:"$rootPass"
openssl pkcs12 -in certs/rootCA.p12 -nokeys -nodes -out certs/test/rootCA.pem -passin pass:"$rootPass"
